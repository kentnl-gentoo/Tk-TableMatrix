...
/* perltk debug only for picking up forward referenced definitions */
/* #include "myinc.h"  */

/* Perltk: XSETCLIP doesn't Appear to be there for perltk so always
  define this : */
#define NO_XSETCLIP
...
Tk_ConfigSpec tableSpecs[] = {
    ...
    {TK_CONFIG_CALLBACK, "-browsecommand", "browseCommand", "BrowseCommand", "",
    ...
    {TK_CONFIG_STRING, "-colseparator", "colSeparator", "Separator", "\t",
    ...
    {TK_CONFIG_CALLBACK, "-coltagcommand", "colTagCommand", "TagCommand", NULL,
    ...
    {TK_CONFIG_STRING, "-rowseparator", "rowSeparator", "Separator", "\n",
    ...
    {TK_CONFIG_CALLBACK, "-rowtagcommand", "rowTagCommand", "TagCommand", NULL,
     Tk_Offset(Table, rowTagCmd), TK_CONFIG_NULL_OK },
    {TK_CONFIG_SYNONYM, "-selcmd", "selectionCommand", (char *)NULL,
     (char *)NULL, 0, TK_CONFIG_NULL_OK},
    {TK_CONFIG_CALLBACK, "-selectioncommand", "selectionCommand",
    ...
    {TK_CONFIG_SCALARVAR, "-variable", "variable", "Variable", (char *)NULL,
...
/* Don't Need This becuase arrays of Objects equivalent to arrays of Args???? 
static Arg *
...
{
    int i;
    Arg *args; /* This needs to not be autoconverted to a Arg, so change the name to argsv 
    ...
    Tcl_Obj *CONST objv[];	/* Argument objects. */
{
    ...
    tablePtr->widgetCmd	= Lang_CreateWidget(interp,
	    tablePtr->tkwin, TableWidgetObjCmd,
				 ...
				 * for initial configuration */
{
    ...
    Tcl_DString error;
    Tk_FontMetrics fm;
    char *currentVarString;        /* Name of the current array variable */
    char *oldVarString = NULL;     /* Name of the old array variable */
    ...
    /* argv = StringifyObjects(objc, objv); */ /* Don't Need this because arrays of Args like arrays of Obj?? */
    objArray = (Tcl_Obj **) objv; /* Cast gets rid of warnings */
    
    result = Tk_ConfigureWidget(interp, tablePtr->tkwin, tableSpecs,
				objc, objArray, (char *) tablePtr, flags);
    /* ckfree((char *) argv); */
    if (result != TCL_OK) {
	return TCL_ERROR;
    }
	...
	if (tablePtr->arrayVar != NULL) {
		...
		/* Tcl_UnsetVar2(interp, currentVarString, TEST_KEY,
			      TCL_GLOBAL_ONLY); */
...
TableAdjustParams(register Table *tablePtr)
{
	...
	if (tablePtr->yScrollCmd != NULL) {
	    ...
	    if ( LangDoCallback(interp,tablePtr->yScrollCmd, 0,2, " %g %g", first, last)
		 != TCL_OK) {
	...
	if (tablePtr->xScrollCmd != NULL) {
	    ...
	    if ( LangDoCallback(interp,tablePtr->xScrollCmd, 0,2, " %g %g", first, last)
		 != TCL_OK) {
		Tcl_AddErrorInfo(interp,
...
#pragma export on
#endif
#if 0 /* perltk This not needed by perlTk */
EXTERN int
Tktable_Init(interp)
     Tcl_Interp *interp;
{
